...
/**
 * Decorative layers using only brand yellow + light accents on navy — pointer-events none.
 */
export default function ReflectBackdrop({ variant, className = "" }: ReflectBackdropProps) {
  ...
  return (
    <div
      className={`pointer-events-none absolute inset-0 overflow-hidden opacity-[0.35] ${className}`}
      aria-hidden
    >
      <svg className="absolute -bottom-8 left-0 h-64 w-full text-sunburst/12 md:h-80" viewBox="0 0 1200 200" preserveAspectRatio="none">
        <defs>
      ...
      {variant === "sun" && (
        <svg className="absolute right-[8%] top-8 h-40 w-40 text-sunburst/25 md:h-52 md:w-52" viewBox="0 0 100 100">
      ...
      {showTrees && (
        <div className="absolute bottom-0 left-0 right-0 text-sunburst/15">
      ...
      {extraTrees && (
        <div className="absolute bottom-0 left-0 right-0 text-sunburst/10">
      ...
      {(variant === "mist" || variant === "dew" || variant === "dawn") && (
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_15%,rgba(252,209,22,0.12),transparent_55%)]" />
      )}

      {variant === "ember" && (
        <svg className="absolute bottom-20 left-[8%] h-36 w-36 text-sunburst/12" viewBox="0 0 100 100">
          <path fill="currentColor" d="M50 95 C20 70 25 40 50 15 C75 40 80 70 50 95Z" />
        </svg>
      )}

      {variant === "dawn" && (
        <svg className="absolute left-[10%] top-14 h-28 w-28 text-sunburst/18" viewBox="0 0 60 60">
...
function LeafScatter({ sparse, floating }: { sparse?: boolean; floating?: boolean }) {
  const n = sparse ? 8 : 14;
  return (
    <svg className="absolute inset-0 text-sunburst/10" viewBox="0 0 400 320" preserveAspectRatio="none">
