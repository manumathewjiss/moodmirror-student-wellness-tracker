...
export default function ReflectBackdrop({ variant, className = "" }: ReflectBackdropProps) {
  ...
  return (
    <div
      className={`pointer-events-none absolute inset-0 overflow-hidden opacity-[0.22] ${className}`}
      aria-hidden
    >
      <svg className="absolute -bottom-8 left-0 h-64 w-full text-emerald-600/18 md:h-80" viewBox="0 0 1200 200" preserveAspectRatio="none">
        <defs>
      ...
      {variant === "sun" && (
        <svg className="absolute right-[8%] top-8 h-40 w-40 text-amber-500/18 md:h-52 md:w-52" viewBox="0 0 100 100">
      ...
      {showTrees && (
        <div className="absolute bottom-0 left-0 right-0 text-emerald-600/18">
      ...
      {extraTrees && (
        <div className="absolute bottom-0 left-0 right-0 text-emerald-700/14">
      ...
      {(variant === "mist" || variant === "dew" || variant === "dawn") && (
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_15%,rgba(255,255,255,0.35),transparent_55%)]" />
      )}

      {variant === "ember" && (
        <svg className="absolute bottom-20 left-[8%] h-36 w-36 text-rose-400/14" viewBox="0 0 100 100">
          <path fill="currentColor" d="M50 95 C20 70 25 40 50 15 C75 40 80 70 50 95Z" />
        </svg>
      )}

      {variant === "dawn" && (
        <svg className="absolute left-[10%] top-14 h-28 w-28 text-teal-500/14" viewBox="0 0 60 60">
...
function LeafScatter({ sparse, floating }: { sparse?: boolean; floating?: boolean }) {
  const n = sparse ? 8 : 14;
  return (
    <svg className="absolute inset-0 text-emerald-500/12" viewBox="0 0 400 320" preserveAspectRatio="none">
